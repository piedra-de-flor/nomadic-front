...
const RecommendScreen = ({ navigation }: any) => {
  // Redux 상태
  ...
  const [sortOrder, setSortOrder] = useState<'createdAt' | 'likesCount' | 'viewsCount' | 'reviewsCount'>('createdAt');
  ...
  useEffect(() => {
    if (!searchQuery.trim() && (sortOrder !== 'createdAt' || sortDirection !== 'DESC')) {
      loadSortedRecommendations();
    }
  }, [sortOrder, sortDirection, searchQuery]);
  ...
  const loadSortedRecommendations = async () => {
    try {
      setIsLoading(true);
      console.log('🔍 정렬된 추천 장소 로드:', { activeTab, sortOrder, sortDirection });
    ...
    try {
      ...
      toggleRecommendationLike(recommendation.id, user.userId)
        .then(result => {
          ...
          //   prev.map(rec => 
          //     rec.id === recommendation.id 
          //       ? { ...rec, likesCount: result.likesCount }
          //       : rec
          //   )
          // );
        })
  ...
  const renderRecommendationItem = ({ item }: { item: Recommendation }) => {
    ...
    // ID 기반으로 일관된 이미지 선택
    const imageIndex = item.id % (isPlace ? placeImages.length : postImages.length);
    const imageUrl = isPlace ? placeImages[imageIndex] : postImages[imageIndex];
            ...
            {isPlace ? (
              ...
              <>
                ...
                <Text style={styles.placeDescription} numberOfLines={2}>
                  {item.blocks?.find(block => block.type === 'TEXT')?.text || ''}
                </Text>
                    ...
                    <View style={styles.authorInfo}>
                      <View style={styles.authorAvatar}>
                        <Text style={styles.authorAvatarText}>
                          {item.postMeta?.author?.charAt(0) || 'U'}
                        </Text>
                      </View>
                      <Text style={styles.authorName}>{item.postMeta?.author || '관리자'}</Text>
                    </View>
